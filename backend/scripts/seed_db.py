...
# --- CONFIGURATION ---
# Set SEED_AUTH_TOKEN in .env or pass via environment
AUTH_TOKEN = os.environ.get('SEED_AUTH_TOKEN', '')
API_ENDPOINT = os.environ.get('SEED_API_URL', 'http://localhost:8005/clips/')
...
def upload_to_echoflow(file_path, title, category):
    ...
    if not AUTH_TOKEN:
        print("❌ SEED_AUTH_TOKEN is not set. Aborting upload.")
        return

    # DRF JWT auth expects "Bearer <token>"
    headers = {
