...
def post_fork(server, worker):
    ...
    # Reset DB connections to prevent sharing across processes
    # Critical because EchoFlow/__init__.py imports Celery app at module load time.
    # When preload_app=True, the Celery app's Redis connections are established
    # in the master process. Without resetting, forked workers inherit stale
    # connections, which leads to "connection reset" errors and silent failures.
    from django.db import connections
    for conn in connections.all():
        conn.close()
    server.log.info(f"Worker spawned (pid: {worker.pid}) - DB connections reset")
