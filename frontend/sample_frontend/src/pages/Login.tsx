...
export function LoginPage({ onSuccess }: Props) {
  const { login, register, devLogin, loading } = useAuth();
    ...
    try {
      ...
      if (e && typeof e === 'object') {
        ...
        if (errData.status === 0) {
          ...
          msg = 'Invalid username or password. Try "dev" / "dev" for quick access.';
  ...
  const quickLogin = async () => {
    ...
    try {
      ...
      let msg = 'Quick login failed';
      if (e && typeof e === 'object') {
        const errData = e as { status?: number; message?: string };
        if (errData.status === 0) {
          msg = 'Cannot connect to backend. Is the server running?';
        } else if (errData.status === 404) {
          msg = 'Dev login is disabled (DEBUG=False). Use username/password login instead.';
        } else {
          msg = errData.message || 'Quick login failed.';
        }
      } else if (e instanceof Error) {
        msg = e.message;
      }
    ...
    <div style={{
      ...
      <div className="fade-up" style={{ width: '100%', maxWidth: 380, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          {DEMO_MODE && (
            <p style={{ fontSize: 11, color: 'var(--terracotta)', marginTop: 8, fontWeight: 600 }}>
              DEMO MODE ACTIVE
            </p>
          )}
        </div>
        ...
        <div style={{
          ...
          {/* Quick login */}
          <button onClick={quickLogin} disabled={loading} style={{
            width: '100%', marginTop: 10, padding: '10px 0', borderRadius: 'var(--radius-full)',
            border: '1px solid var(--outline-variant)',
            background: 'var(--surface-lowest)',
            color: 'var(--on-surface-variant)', fontSize: 12, fontWeight: 700,
            letterSpacing: '0.06em', cursor: loading ? 'default' : 'pointer', transition: 'all 0.2s'
          }}>
            QUICK LOGIN (dev/dev)
          </button>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20, padding: '0 4px' }}>
          ...
          {DEMO_MODE && (
            <button onClick={() => onSuccess('devdemo')} style={{
              padding: '8px 16px', borderRadius: 'var(--radius-full)', fontSize: 12, fontWeight: 700,
